...
bool CollisionDetection::GJK(GameObject* gm1_, GameObject* gm2_)
{
	...
	for (auto v : m1->GetSubMeshes())
	{
		...
		for (auto v2 : m2->GetSubMeshes())
		{
			...
			if (GJKCalculations(gm1_->GetModelInstanceTransform(),gm2_->GetModelInstanceTransform(), vl1, vl2))
			{
				return true;
			}
			else
...
bool CollisionDetection::GJKCalculations(glm::mat4 t1_, glm::mat4 t2_, std::vector<Vertex> v1_, std::vector<Vertex> v2_)
{
	...
	{
		add1 += v1_[i].position;//+ glm::vec3(t1_[3].x, t1_[3].y, t1_[3].z);
		v1_[i].position += glm::vec3(t1_[3].x, t1_[3].y, t1_[3].z);
	}
	for (int i = 0; i < v2_.size(); i++)
	{
		add2 += v2_[i].position;//+ glm::vec3(t1_[3].x, t1_[3].y, t1_[3].z);
		v2_[i].position += glm::vec3(t2_[3].x, t2_[3].y, t2_[3].z);
	}

	//std::cout << "add1 positions " << add1.x << "  " << add1.y << "  " << add1.z << "\n\n";
	//std::cout << "add2 positions " << add2.x << "  " << add2.y << "  " << add2.z << "\n\n";
	glm::vec3 c1 = add1 / (float)v1_.size();
	c1+= glm::vec3(t1_[3].x, t1_[3].y, t1_[3].z);
	glm::vec3 c2 = add2 / (float)v2_.size();
	c2+= glm::vec3(t2_[3].x, t2_[3].y, t2_[3].z);

	//std::cout << "c1 positions " << c1.x << "  " << c1.y << "  " << c1.z << "\n\n";
	//std::cout << "c2 positions " << c2.x << "  " << c2.y << "  " << c2.z << "\n\n";
	...
	while (true)
	{
		...
		if (simplex.size() > 0 && glm::dot(simplex.back(), dir) <= 0)
		{
			...
			std::cout << "Second dot " << glm::dot(simplex.back(), dir) << std::endl;
			return false;
		}
		else
		{
...
glm::vec3 CollisionDetection::FindFurthestPointInDirection(std::vector<Vertex>v_, glm::vec3 dir_)
{
	...
	{
		float calc = glm::dot(v_[i].position, dir_);
		if (calc >= max)
		{
			max = calc;
			index = i;
...
bool CollisionDetection::ContainsOrigin(glm::vec3 p1_, glm::vec3 p2_, glm::vec3 p3_, glm::vec3 p4_)
{
	...
	float det3;
	float det4;
	std::cout << "P1x " << p1_.x << " P1y " << p1_.y << " P1z " << p1_.z << "\n";
	std::cout << "P2x " << p2_.x << " p2y " << p2_.y << " p2z " << p2_.z << "\n";
	std::cout << "P3x " << p3_.x << " p3y " << p3_.y << " p3z " << p3_.z << "\n";
	std::cout << "P4x " << p4_.x << " p4y " << p4_.y << " p4z " << p4_.z << "\n";
